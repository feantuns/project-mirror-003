export default class BloomFilter {
  ...
  mayContain(item) {
    ...
    for (let hashIndex = 0; hashIndex < hashValues.length; hashIndex += 1) {
      if (!this.storage.getValue(hashValues[hashIndex])) {
        // We know that the item was definetly not inserted.
        return false;
      }
    }
   ...
   * encapsulate the data itself and only provide acces
  ...
  /**
   * Run all 3 hash functions on the input and returns an array of results.
   *
   * @param {string} item
   * @return {number[]}
   */
